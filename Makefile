...
KIND_CLUSTER ?= nomad-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name nomad-operator-builder
	$(CONTAINER_TOOL) buildx use nomad-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm nomad-operator-builder
	rm Dockerfile.cross
...
test-integration: ## Run hermetic Nomad integration tests (requires a nomad v2.0.4 binary in PATH; may need elevated privileges on Linux).
	go test -tags integration ./internal/nomad/... -run 'TestDevAgent|TestACLBootstrapAndLeaderLive|TestNodeEligibilityAndDrainLive|TestNodePoolLifecycleLive|TestJobLifecycleLive' -v
